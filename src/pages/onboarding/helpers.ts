...
type EducationalExperience = NonNullable<IntakeFormData["educationalExperience"]>;

export const parseEducationRows = (education?: EducationalExperience): EducationRow[] => {
  ...
  const length = Math.max(schools.length, dates.length, qualifications.length, 1);
...
export const rowsToEducationFields = (rows: EducationRow[]) => ({
  schoolsAttended: rows.map((r) => r.school).filter(Boolean).join("\n"),
  dates: rows.map((r) => r.date).filter(Boolean).join("\n"),
  qualificationsObtained: rows.map((r) => r.qualification).filter(Boolean).join("\n"),
});
...
export const validateStep = (
  ...
  if (key === "personal") {
    if (!isFilled(intakeForm.personalInfo?.mailingCity)) missing.push("Mailing City");
    if (!isFilled(intakeForm.personalInfo?.mailingState)) missing.push("Mailing State");
    ...
    if (!isFilled(intakeForm.personalInfo?.dateOfBirth)) missing.push("Date of Birth");
    ...
    if (!isFilled(intakeForm.personalInfo?.nationality)) missing.push("Nationality");
    if (!isFilled(intakeForm.personalInfo?.ethnicOrigin)) missing.push("Ethnic Origin");
    if (!isFilled(intakeForm.personalInfo?.homePhone)) missing.push("Home Phone");
  ...
  if (key === "spiritual") {
    if (!isFilled(intakeForm.spiritualExperience?.bornAgain)) missing.push("Born Again");
    if (
    ...
    if (!isFilled(intakeForm.spiritualExperience?.churchName)) missing.push("Church Name");
  ...
  if (key === "education") {
    educationRows.forEach((row, idx) => {
      ...
      if (!isFilled(row.qualification)) missing.push(`Qualification ${idx + 1}`);
    });
  }

  if (key === "employment") {
    if (!isFilled(intakeForm.employmentStatus?.status)) missing.push("Employment Status");
    if (!isFilled(intakeForm.employmentStatus?.familyAnnualIncome)) {
      missing.push("Family Annual Income");
    }
  }

  if (key === "declaration") {
    if (!isFilled(intakeForm.declaration?.agreed)) missing.push("Declaration Agreement");
  }
