import { Chip, Typography, Stack, Select, Option, FormControl, FormLabel, Textarea } from "@mui/joy";
import { useState } from "react";
import { toast } from "react-toastify";
import Frame from "../../components/frame/Frame";
import AppButton from "../../components/Button/AppButton";
import {
  useGetPublicationsQuery, useApprovePublicationMutation,
  useRejectPublicationMutation, useLockResultsMutation,
  useGetSessionsQuery, useGetTermsQuery,
} from "../../data/rtk/academic";
import { PUB_STATUS_COLOR, resolveName } from "../../utils/academic";
...
const PublicationsPage = () => {
  ...
  const handleLock = async () => {
    ...
    try {
      await lock({ sessionId: lockSessionId, ...(lockTermId ? { termId: lockTermId } : {}) }).unwrap();
    ...
    <Frame text="Publications Queue">
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <div className="bg-gradient-to-br from-[#001F54] to-[#001EC5] rounded-2xl p-6 text-white">
          <Typography level="body-sm" sx={{ color: "#bfdbfe" }}>Pending Approvals</Typography>
          <Typography level="h2" sx={{ color: "white", fontWeight: 900, my: 1 }}>{pending}</Typography>
          <Typography level="body-xs" sx={{ color: "#93c5fd" }}>
            {pending > 0 ? "Action required — review below" : "All clear, no pending submissions"}
          </Typography>
        </div>

        <div className="bg-white border border-[#E6ECFF] rounded-2xl p-6 grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormControl size="sm">
              ...
              <Select placeholder="Session" value={lockSessionId} onChange={(_, v) => setLockSessionId(v as string)}>
                {sessions.map((s) => <Option key={s._id} value={s._id}>{s.name}</Option>)}
              </Select>
            </FormControl>
            <FormControl size="sm">
              <FormLabel>Term (optional)</FormLabel>
              <Select placeholder="All terms" value={lockTermId} onChange={(_, v) => setLockTermId(v as string)}>
                <Option value="">All terms</Option>
                {terms.map((t) => <Option key={t._id} value={t._id}>{t.name}</Option>)}
              </Select>
            </FormControl>
          </div>
          <AppButton onClick={handleLock} loading={locking} variant="outlined">
            Lock Results
          </AppButton>
        </div>
      </div>
      ...
      {/* Filter */}
      <Stack direction="row" gap={2} mt={5} alignItems="flex-end" flexWrap="wrap">
        <Typography level="title-lg" sx={{ mr: "auto" }}>Submission History</Typography>
        <FormControl size="sm" sx={{ minWidth: 160 }}>
          <FormLabel>Status</FormLabel>
          <Select value={statusFilter} onChange={(_, v) => setStatusFilter((v as PublicationStatus | "") ?? "")}>
      ...
      {/* Table */}
      <div className="bg-white rounded-2xl border border-[#E6ECFF] shadow-sm overflow-x-auto mt-4">
        <table className="w-full text-sm text-left text-[#001F54]">
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={7}><TableSkeleton columns={7} rows={5} /></td></tr>
            ) : publications.length === 0 ? (
              <tr><td colSpan={7}><CenteredEmptyState description="No publication requests found." /></td></tr>
            ) : (
              publications.map((p) => {
                const sub = p.submittedBy as any;
                return (
                  <tr key={p._id} className="border-t border-[#F1F5F9] hover:bg-[#F5FAFF]">
                    <td className="px-5 py-3 font-medium">{resolveName(p.centerId as any)}</td>
                    <td className="px-5 py-3">{resolveName(p.sessionId as any)}</td>
                    <td className="px-5 py-3">{resolveName(p.termId as any)}</td>
                    ...
                    <td className="px-5 py-3">
                      <Chip color={PUB_STATUS_COLOR[p.status]} variant="soft" size="sm">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl grid gap-4">
            ...
            <Stack direction="row" gap={2} justifyContent="flex-end">
              <AppButton variant="outlined" onClick={() => setRejectId(null)}>Cancel</AppButton>
              <AppButton color="danger" onClick={handleReject} loading={actionId === rejectId}>
                Confirm Reject
              </AppButton>
            </Stack>
          </div>
