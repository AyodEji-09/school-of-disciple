import { Select, Option, Typography, Stack, FormControl, FormLabel, Alert } from "@mui/joy";
...
import {
  useGetSessionsQuery, useGetTermsQuery, useBulkUploadResultsMutation,
...
const BulkUploadPage = () => {
  ...
  // Only load terms for the selected session
  ...
  const [result, setResult] = useState<null | { total: number; processed: number; errors: number; details: any }>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Processed ${(res as any).data?.processed} of ${(res as any).data?.total} rows`);
    } catch (err) {
  ...
  return (
    <Frame text="Bulk Upload Results">
      <div className="max-w-2xl mx-auto mt-6 grid gap-6">
        {/* Instructions + Template Download */}
        <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl p-5">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <Typography level="title-sm" textColor="#1d4ed8" mb={1}>📋 File Format Guide</Typography>
              <Typography level="body-sm" textColor="#1e40af">
                Upload an <strong>.xlsx</strong> or <strong>.csv</strong> file. Required columns:
              </Typography>
              <ul className="mt-2 text-sm text-[#1e40af] list-disc list-inside space-y-1">
                <li><code>matricNumber</code> or <code>email</code> to identify each student</li>
                <li>One column per subject using the subject <strong>code</strong> (e.g. <code>MATH101</code>), score 0–100</li>
              </ul>
            </div>
            <AppButton
              variant="outlined"
        ...
        <div className="bg-white rounded-2xl border border-[#E6ECFF] shadow-sm p-8 grid gap-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormControl required>
              ...
              <Select
                ...
                value={sessionId}
                onChange={(_, v) => { setSessionId(v as string); setTermId(""); }}
              >
                {sessions.map((s) => <Option key={s._id} value={s._id}>{s.name}</Option>)}
              </Select>
            </FormControl>

            {/* Term — only shows after session is chosen */}
            <FormControl required>
          ...
          <div
            className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors ${
              dragOver ? "border-[#001EC5] bg-[#EFF6FF]" : "border-[#CBD5E1] hover:border-[#001EC5]"
            }`}
          ...
          >
            ...
            <Typography level="body-md" textColor="neutral.600">
              {file ? (
                <span className="font-semibold text-[#001F54]">{file.name}</span>
              ) : (
                <>Drag & drop your file here, or <span className="text-[#001EC5] font-semibold">browse</span></>
              )}
            </Typography>
            <Typography level="body-xs" textColor="neutral.400" mt={0.5}>
          ...
          <Stack direction="row" gap={2} justifyContent="flex-end">
            ...
            <AppButton onClick={handleSubmit} loading={isLoading}>Upload & Process</AppButton>
          </Stack>
        </div>

        {/* Results panel */}
        {result && (
          <div className="bg-white rounded-2xl border border-[#E6ECFF] shadow-sm p-6 grid gap-4">
            <Typography level="title-md">Upload Report</Typography>
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={label} className="bg-[#F5FAFF] rounded-xl p-4 text-center">
                  <Typography level="h3" sx={{ color, fontWeight: 800 }}>{value}</Typography>
            ...
            {result.details?.errors?.length > 0 && (
              <div>
                <Typography level="title-sm" textColor="danger.600" mb={1}>
                ...
                <div className="max-h-48 overflow-y-auto border border-red-100 rounded-lg divide-y divide-red-50">
                  {result.details.errors.map((e: any, i: number) => (
                    <div key={i} className="px-4 py-2 text-sm text-red-700">
                      Row {e.row}: {e.error} {e.studentId ? `(${e.studentId})` : ""}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {result.processed > 0 && (
              <AppButton onClick={() => navigate("/dashboard/results")}>
